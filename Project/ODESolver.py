...
class fixed_point():
    def __init__(self, f, initial_guess, tolerance=1e-6, max_iterations=100) -> None:
        self.f = f
        self.initial_guess = initial_guess
        self.tolerance = tolerance
        self.max_iterations = max_iterations

        # self.x = self.fixed_point_iteration()

    def compute(self, extra_params):
        """
        Solves the equation x = f(x) using fixed-point iteration.
        
        Parameters:
        - f: The function representing the equation x = f(x).
        - initial_guess: Initial guess for the solution.
        - tolerance: Tolerance level for stopping the iteration.
        ...
        for _ in range(self.max_iterations):
            x_new = self.f(x, *extra_params)
            if abs(x_new - x) < self.tolerance:
                return x_new  # Convergence achieved
            x = x_new
        raise ValueError(f'Fixed-point iteration did not converge within the specified number of iterations: last x={x}')
